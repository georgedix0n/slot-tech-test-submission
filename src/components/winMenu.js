...
export class WinMenu {
	...
	_initializeWinMenu() {
		...
		this._symbols.forEach((symbol, index) => {
			const singleSymbolValueContainer = new PIXI.Container();

			const xPosition = index * symbolContainerWidth + index * symbolPadding;

			singleSymbolValueContainer.x = xPosition;

			const instructionSymbol = PIXI.Sprite.from(`${symbol.name}Win1`);
			...
			const valueText = new PIXI.Text(symbol.value, {
				fontSize: 24,
				fontFamily: "Arial",
				fill: 0xffffff,
			});
		...
		uniqueWinMultipliers.forEach((winMultiplier, index) => {
			const singleWinMultiplierContainer = new PIXI.Container();

			const xPosition =
				index * winMultiplierContainerWidth + index * symbolPadding;
			singleWinMultiplierContainer.x = xPosition;

			const nameText = new PIXI.Text(`${winMultiplier.name}:`, {
				fontFamily: "Arial",
				fontSize: 24,
				fill: 0xffffff,
				wordWrap: true,
			...
			const valueText = new PIXI.Text(`x${winMultiplier.value}`, {
				fontSize: 24,
				fontFamily: "Arial",
				fill: 0xffffff,
			});
